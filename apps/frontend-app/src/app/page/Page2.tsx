...
const Page2: React.FC = () => {
  ...
  const [machineStates, setMachineStates] = useState<{
    [key: string]: { status: boolean; records: string[] };
  }>(
    // 초기 상태 설정: 기계 번호 및 상태(false: OFF, true: ON), 작업 기록 배열
    Object.fromEntries(
      Array.from({ length: 5 }, (_, index) => [
        `machine${index + 1}`,
        { status: false, records: [] },
      ])
    )
  );
  ...
  const toggleAllMachines = () => {
    // 모든 기계가 ON 상태인지 확인
    const allMachinesOn = Object.values(machineStates).every(
  ...
  return (
    <div className="page2-container">
      <button className="toggle-all-button" onClick={toggleAllMachines}>
        {Object.values(machineStates).every((machine) => machine.status)
          ? '일괄 종료'
          : '일괄 시작'}
      </button>
          ...
          <li key={machine} className="machine-item">
            ...
            <button
              ...
              onClick={() =>
                addRecord(
                  machine,
                  `작업 기록 추가: ${new Date().toLocaleString()}`
                )
              }
            >
              기록 추가
